...
import highlight from "./../../public/images/mobileappdevelopment/highlight.png"
import Aos from "aos"
...
const HighlightedFeatures = ({heading,paragraph}) => {
  ...
  return (
    <section className="flex max-sm:flex-col items-center justify-between w-[80%] py-20  mx-auto lg:flex">
      <div className="w-full pr-8 lg:w-1/2 max-sm:w-[100%]">
        ...
        <div className="space-y-4">
          {features.map((feature, index) => (
            <div key={index}>
              <div
                className="flex items-center justify-between py-3 border-b text-[#51b0fd] cursor-pointer"
                onClick={() => toggleFeature(index)}
              >
                <span className="text-lg font-semibold text-[#020842]">{feature.title}</span>
                <span className='text-[#2196F3]'>{openFeature === index ? '-' : '+'}</span>
              </div>
              {openFeature === index && (
                <div className="p-4 text-[#616161] border-l-4 border-[#2196F3]">
                  {feature.content}
                </div>
              )}
            </div>
      ...
      <div className="flex justify-center w-full mt-8 lg:w-1/2 lg:mt-0 max-sm:w-[100%]">
